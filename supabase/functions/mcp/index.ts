...
var get_app_info_default = defineTool({
  ...
  description:
    'Returns overview information about the VISOR AI fitness app, including its purpose, key features, and subscription tiers.',
  inputSchema: {},
  annotations: {
    readOnlyHint: true,
    idempotentHint: true,
...
var list_features_default = defineTool2({
  ...
  description:
    "Lists VISOR's main product features, optionally filtered by category (coaching, transformation, tracking, all).",
  inputSchema: {
    category: z
  ...
  handler: ({ category }) => {
    ...
    const chosen =
      category === 'all'
        ? Object.entries(features).flatMap(([k, v]) =>
            v.map((f) => `[${k}] ${f}`),
          )
        : features[category]
    return {
...
import mcp from 'npm:C:\\Users\\hp\\Desktop\\veynta projects\\visor-experience-studio\\src\\lib\\mcp\\index.ts'
