...
const EXCLUDED_PATHS = new Set(["*", "/not-found", "/404"]);
...
const HINTS = {
  ...
  "/delete-account": { changefreq: "yearly", priority: "0.3" },
...
export function discoverRoutes(appPath = resolve("src/App.tsx")) {
...
export function buildEntries(routes) {
...
function main() {
  ...
  if (isCheck) {
    if (!existsSync(sitemapPath)) {
      console.error("public/sitemap.xml is missing. Run: node scripts/generate-sitemap.mjs");
      process.exit(1);
    }
